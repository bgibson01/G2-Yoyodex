  ...
  const CONFIG = {
    ...
    async function fetchSpecsData() {
      try {
        ...
        const data = await response.json();
        console.log('Fetched Specs Data:', data);
        return data;
      } catch (error) {
    ...
    async function displayYoyoCards(yoyoData) {
      const specsData = await fetchSpecsData(); // Fetch the specs data
  
      yoyoGrid.innerHTML = '';  // Clear the grid before adding new cards
  
      yoyoData.forEach(yoyo => {
        const yoyoCard = document.createElement('div');
        yoyoCard.classList.add('yoyo-card', 'bg-gray-800', 'p-4', 'rounded', 'shadow-lg');
        ...
        showSpecsButton.addEventListener('click', () => showSpecs(yoyo.model, specsData));
    ...
    function showSpecs(model, specsData) {
      const specs = specsData.find(spec => spec.model === model);
  
      if (specs) {
        alert(`Specs for ${model}: ${JSON.stringify(specs, null, 2)}`); // Customize this to show specs in a better format
      }
    }
